...
class gui(dummyGUIfuncs):
    ...
    def __init__(s, guiType, version, customFrameInsert: dict = {}):
        ...
        if not guiType or guiType == "tk": 
            ...
            s.makeHelperInfo = s._TKmakeHelperInfo
            s.makeText = s._TKmakeText
        elif guiType == "ctk":
            global _TK
            global _CTK
            from tkinter import Tk as _TK
            ...
            s.makeHelperInfo = s._CTKmakeHelperInfo
            s.makeText = s._CTKmakeText
            ...
            s.makeHelperInfo = s._DPGmakeHelperInfo
            s.makeText = s._DPGmakeText
        elif guiType == "$custom":
            ...
            s.makeHelperInfo = customFrameInsert["makeHelperInfo"]
            s.makeText = customFrameInsert["makeText"]
    ...
    def _TKmakeText(s, holder, text = "", x = 0, y = 0, others = []):
        _text = _TKINTER.Label(holder, text = text)
        _text.place(x = x, y = y)
        if "helperInfo" in others: s.ANYhelperText[holder] = _text
        return _text
    def _CTKmakeText(s, holder, text = "", x = 0, y = 0, others = []):
    ...
    def _TKmakeHelperInfo(s, target, info):
        """
        target: _TKINTER.Widget
        """
        target.bind("<Enter>", lambda a: s._TKmakeText(target.master, info, 20, target.master.winfo_height() - 30, ["helperInfo"]))
        target.bind("<Leave>", lambda a: s.ANYhelperText[target.master].destroy())
    def _CTKmakeHelperInfo(s, target, info):
        """
        target: _CTK.CTkBaseClass
        """
